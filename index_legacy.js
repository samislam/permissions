...
const getValue = async (parameter, ...args) =>
  checkTypes.isAsycOrSyncFunc(parameter) ? await parameter(...args) : parameter

function permissionsMw(permission, options) {
  return expressAsyncHandler(async (req, res, next) => {
    // @param perrmission: function
...
async function permissions(permission, options) {
  ...
  const defaultOptions = {
    ...
    denyStatusCode: 401,
    defaultBehaviour() {
      throw new AppError(chosenOptions.denyMsg, chosenOptions.denyStatusCode, 'permission_denied_err')
    },
  }
  _.merge(chosenOptions, defaultOptions, optionsValue)
